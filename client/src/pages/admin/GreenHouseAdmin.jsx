...
export default function GreenHouseAdmin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/v1/greenhouse/create-greenhouse`,
  ...
  const handleDelete = async (gid) => {
    try {
      const { data } = await axios.delete(
        `${process.env.REACT_APP_API_URL}/api/v1/greenhouse/delete-greenhouse/${gid}`
      );
      if (data.success) {
      ...
      console.log(error);
      toast.error("Something went wrong while deleting greenhouse");
    }
  };
  ...
  return (
    <AdminLayout>
      <div className="w-full flex flex-col h-full overflow-auto">
        <h2 className="text-2xl font-semibold">ग्रीनहाउस</h2>
