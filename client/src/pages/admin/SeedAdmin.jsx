  ...
  const getAllSeed = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/v1/seed/get-seed`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/v1/seed/create-seed`,
  ...
  const handleDelete = async (sid) => {
    try {
      const { data } = await axios.delete(
        `${process.env.REACT_APP_API_URL}/api/v1/seed/delete-seed/${sid}`
      );
      if (data.success) {
  ...
  return (
    <AdminLayout>
      <div className="w-full flex flex-col h-full overflow-auto">
        <h2 className="text-2xl font-semibold">बीज भंडारित करें</h2>
