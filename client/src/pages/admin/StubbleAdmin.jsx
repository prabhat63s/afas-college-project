  ...
  const getAllStubble = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/v1/stubble/get-stubble`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${process.env.REACT_APP_API_URL}/api/v1/stubble/create-stubble`,
  ...
  const handleDelete = async (sid) => {
    try {
      const { data } = await axios.delete(
        `${process.env.REACT_APP_API_URL}/api/v1/stubble/delete-stubble/${sid}`
      );
      if (data.success) {
